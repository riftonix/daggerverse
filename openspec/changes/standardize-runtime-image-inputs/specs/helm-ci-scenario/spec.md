...
- **THEN** the scenario SHALL use `helm_image_registry`, `helm_image_repository`, `helm_image_tag`, and `helm_user_id` when creating the Helm module
...
- **THEN** the scenario SHALL use `git_image_registry`, `git_image_repository`, `git_image_tag`, and `git_user_id` when creating the Git module
