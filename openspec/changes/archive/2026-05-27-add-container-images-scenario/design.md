...
- Keep scenario public APIs independent from implementation-module object types so users can compose released scenarios and modules without type collisions.
...
- Do not expose Docker module result objects, such as `DockerBuild` or `DockerImage`, from the container-images scenario public API.
...
5. Scenario APIs will hide implementation-module object types.

   The repository treats modules as leaf building blocks and scenarios as compositions of modules, scenarios, or both. A scenario may depend on `modules/docker` internally, but its public API should accept and return stable Dagger primitives or scenario-owned spec/result objects. It should not accept or return `Docker`, `DockerBuild`, or `DockerImage`. This keeps a released scenario self-consistent while still letting users compose their own scenarios from a different released Docker module version when they need to.
...
9. The Docker module will expose `Docker`, `DockerBuild`, and `DockerImage` objects.
...
10. The Docker module will use `with_registry_auth`.
...
11. The Docker module will publish through Dagger-native `Container.publish`.
...
12. The Docker module will accept target platforms.
...
13. Build arguments will use CLI-friendly `KEY=VALUE` strings.
...
- Users may compose a released scenario and a different released version of an implementation module in their own scenario -> Keep scenario public APIs module-neutral and pin scenario dependencies internally.
