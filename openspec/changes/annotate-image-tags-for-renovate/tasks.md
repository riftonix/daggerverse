...
- [ ] 3.1 In `scenarios/helm-ci/src/helm_ci/main.py`, add a Renovate annotation before `DEFAULT_HELM_IMAGE_TAG` with `datasource=docker depName=alpine/helm`.
- [ ] 3.2 In `scenarios/helm-ci/src/helm_ci/main.py`, add a Renovate annotation before `DEFAULT_GIT_IMAGE_TAG` with `datasource=docker depName=alpine/git`.
- [ ] 3.3 In `scenarios/helm-ci/tests/src/tests/main.py`, add Renovate annotations before `DEFAULT_HELM_IMAGE_TAG`, `DEFAULT_GIT_IMAGE_TAG`, and `FIXTURE_GIT_IMAGE_TAG` with dependency names `alpine/helm`, `alpine/git`, and `alpine/git` respectively.
...
- [ ] 3.5 In `scenarios/static-site/tests/src/tests/main.py`, add a Renovate annotation before `DEFAULT_HUGO_IMAGE_TAG` with `datasource=docker depName=ghcr.io/riftonix/container-images/hugo-autoprefixer`.
- [ ] 3.6 Confirm `scenarios/container-images/src/container_images/main.py` has no runtime image defaults to annotate and leave it unchanged unless a managed runtime image default is found during implementation.

## 4. Consistency Checks

- [ ] 4.1 Verify every managed `*_IMAGE_TAG` constant has the Renovate annotation on the immediately preceding line.
- [ ] 4.2 Verify every annotation `depName` matches the adjacent image registry and repository convention: Docker Hub images use repository names such as `alpine/helm`, and GHCR images include `ghcr.io/`.
- [ ] 4.3 Verify matching images use identical `datasource` and `depName` across modules, scenarios, and tests so Renovate can update them in one MR.
- [ ] 4.4 Verify no runtime image defaults remain as inline string literals in function signatures or constructor fallbacks for the covered modules and scenarios.
- [ ] 4.5 Update the relevant `docs/` page during archive to document the image constants and Renovate annotation convention for new modules, scenarios, and tests.

## 5. Validation

- [ ] 5.1 Run a JSON/schema validation for `renovate.json`.
- [ ] 5.2 Run Python linting or formatting checks required by the repository for changed Python files.
- [ ] 5.3 Run the relevant OpenSpec validation command for `annotate-image-tags-for-renovate`.
- [ ] 5.4 Do not run heavy Dagger integration tests locally; provide the exact command for the user to run if full Dagger verification is needed.
