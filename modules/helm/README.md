...
For full repository documentation, see [../../docs/README.md](../../docs/README.md). For Helm examples, start with [../../docs/tutorials/get-started-with-helm.md](../../docs/tutorials/get-started-with-helm.md).
...
async def main():
    async with dagger.Connection(dag) as client:
        chart_dir = client.host().directory("./modules/helm/tests/charts/ns-configurator")
...
```bash
dagger -m ./modules/helm call lint --source=./charts/mychart --strict=true
```

Render templates:

```bash
dagger -m ./modules/helm call template --source=./charts/mychart --release-name=ci-release
```

Package chart:

```bash
dagger -m ./modules/helm call package --source=./charts/mychart --version=0.1.0 --app-version=1.0.0
```
...
dagger -m ./modules/helm call push \
